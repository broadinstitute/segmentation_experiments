...
def random_crop(img, mask_composite=None, crop_size=(64,64), loud=False):
    img = img[:, :, :3]                                            # neglect alpha value
    if isinstance(crop_size, int):
        crop_size = (crop_size, crop_size)
        
    row_start = np.random.randint(0, img.shape[0] - crop_size[0])  # select a random row and column
    col_start = np.random.randint(0, img.shape[1] - crop_size[1])  # while still allowing for a full crop

    row_end = row_start + crop_size[0]
    ...
    if loud:
        print("Original image size is %d by %d. Crop starts at (%d,%d). Cropped image is %dx%d pixels" 
              %(img.shape[0], img.shape[1], row_start, col_start, crop_size[0], crop_size[1]))
